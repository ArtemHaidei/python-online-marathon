...
def file_parser(path_, find_, replace_=None):
    count = 0
    with open(path_, 'r' if replace_ is None else "r+") as file:
        if replace_ is None:
            ...
            for line in file.readlines():
                print(line)
                count += line.count(find_)
                changed_line.append(line.replace(find_, replace_))
            file.seek(0)
            file.writelines(changed_line)
    return f'Found {count} strings' if replace_ is None else f'Replaced {count} strings'
